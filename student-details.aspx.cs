        ...
        if (!this.IsPostBack)
        {
            ...
            try
            {
                using (MySqlConnection con = new MySqlConnection(constr))
                {
                    ...
                    string strcmd = "SELECT A.fName, A.Lname, A.emailId, B.courseId, B.courseStatus from tbluser as A " +
                        "LEFT OUTER JOIN tblregistration as B on B.userId = A.userId where A.userId='" + userId + "'";
                    ...
                    if (dr.HasRows)
                    {
                        dr.Read();
                        {
                            ...
                            lblCourseApprove.Text = dr["courseStatus"].ToString();
                        }
                        dr.Close();
                    }
                    else
